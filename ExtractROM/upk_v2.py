...
def extract_folder(folder, path):
    ...
    for file in folder.files:
        if file.name in [".", ".."]:
            continue
        print "{}/{}{}".format(path, file.name.decode("cp932"), "/" if file.flags & 0x80 else "")
        if file.flags & 0x80:
            extract_folder(folder_map[file.offset], os.path.join(path, file.name))
        else:
            fin.seek(file.offset * 0x200)
            fout = open(os.path.join(path, file.name.decode("cp932")), "wb")
